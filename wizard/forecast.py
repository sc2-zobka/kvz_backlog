...
class ForecastWizard(models.TransientModel):
	...
	def action_confirm(self):
		...
		if button == 'forecast':
			...
			backlog_state_id = self.env['backlog.stages'].search([('stages_type','=','forecast')])
			order_line.backlog_state_id = backlog_state_id.id
			order_line.bklg_state = 'forecast'

		elif button == 'Unforecast':
			if activity_type_id:
				vals.update({'res_name':order_line.name +': Unforecast','summary':' Unforecast  => '})
				self.env['mail.activity'].create(vals)
		
		elif button == "send_invoicing":
			backlog_state_id = self.env['backlog.stages'].search([('stages_type','=','planning')])
			order_line.backlog_state_id = backlog_state_id.id
			order_line.bklg_state = 'sent_to_invoicing'
			if activity_type_id:
				vals.update({'res_name':order_line.name +': Sent to Invoicing','summary':' Sent to Invoicing  => '})
				self.env['mail.activity'].create(vals)
			...
			if activity_type_id:
				vals.update({'res_name': order_line.name +': Sent to Provision','summary':' Sent to Provision  => '})
				self.env['mail.activity'].create(vals)
