...
class BacklogStages(models.Model): 
	...
	stages_type = fields.Selection([
			...
			('invoiced', 'Invoiced')
		], default="forecast", string="Stages")

	@api.constrains('stages_type')
	def check_stages_type(self):
		stage_id = self.search([('stages_type', '=', self.stages_type),('id','!=',self.id)])
		stages_dict = {
			'forecast': 'Forecast',
			'planning': 'Planning',
			'provisioned': 'Provisioned',
		...
		if stage_id:
			raise UserError(_("Stage ' %s ' Already Exist !!!") % (stages_dict.get(self.stages_type)))
...
class kvz_backlog_lines(models.Model):
	...
	def get_currency_id(self):
		convert_currency_id = self.env['res.currency'].search([('name','=','CLP')], limit=1)
		return convert_currency_id.id if convert_currency_id else False

	project_manager = fields.Many2one('res.users',string='Project Manager', required = False, related='project_id.user_id',
	...
	current_user_id = fields.Boolean(string='Current Users', compute="compute_get_users")

	country_id = fields.Many2one('res.country',string='Country', related='order_partner_id.country_id',
									readonly=True)
	service_id = fields.Many2one('account.analytic.account', 
									string='Service', ondelete='set null', check_company=True)
	project_id = fields.Many2one(related="order_id.project_id", store=True, string='Project', check_company=True)
	backlog_state_id = fields.Many2one('backlog.stages', string='Stage', required=False, readonly=False, copy=False, tracking=True, group_expand='_read_group_stage_ids')
	...
	clp_price_total = fields.Monetary(string='Price Subtotal CLP', compute="compute_clp_currency", store=True)
	state = fields.Selection(related="order_id.state", store=True, string='Order Status')
	...
	def _get_bklg_state_list(self):
		...
		if self.env.user.has_group('kvz_backlog.group_project_manager') and \
				not self.env.user.has_group('kvz_backlog.group_backlog_users'):
	...
	bklg_state = fields.Selection(string="State", selection="_get_bklg_state_list", copy=False, default=False)

	cpl_currency_id = fields.Many2one('res.currency', string='Currency', default=get_currency_id)
	...
	amount_us = fields.Float(string='Subtotal USD', compute="_compute_amount_us", store=True)
		...
		for rec in self:
			if rec._context.get('manualy_change'):
				if self.order_id.project_line_ids:
					...
					if self.env.user.id not in manager_ids and not self.project_manager.id == self.env.user.id:
						raise UserError(_("You are not allowed to make changes, please contact the Project Manager!"))
	...
	def write(self, vals):
		...
		if vals.get('bklg_state') and vals.get('date_deadline'):
			...
			if new_date != self.date_deadline or vals.get('bklg_state') != self.bklg_state:
				vals.update({'replanning_count': self.replanning_count + 1})
		if vals.get('bklg_state') and vals.get('bklg_state') == 'sent_to_invoicing' and vals.get('bklg_state') != self.bklg_state:
			if not vals.get('date_deadline'):
				raise UserError(_("Please set date deadline first!"))
			activity_type_id = self.env['mail.activity.type'].sudo().search([('is_todo','=',True)], limit=1)
			date_deadline = datetime.strptime(vals.get('date_deadline'), '%Y-%m-%d').date()
			model_id = self.env['ir.model'].sudo().search([('model','=','sale.order.line')], limit=1)
			...
			self.message_post(body=f"<b> Sent to Invoicing by {self.env.user.name}.<br/></b>")
	...
	def _check_validate_status(self):
		for rec in self:
			user = self.env.user
			if not user.has_group('kvz_backlog.group_head_of_sales') and not user.has_group('kvz_backlog.group_business_owner') \
				and rec.project_manager.id != user.id and not user.has_group('kvz_backlog.group_project_manager') and \
				rec.order_id.user_id.id != user.id:
				if rec.state in ['draft', 'sent'] and (rec.backlog_state_id or rec.bklg_state):
					raise ValidationError("""This Backlog Kuvasz only Business Owner or Head of Sale can changes \n Becuase this order status is {status} and 'Estado backlog is blank!!""".format(status=rec.state))
	...
	# cron job for daily currency base update price
	def _schedule_reservation_move(self):
		clp = self.env['res.currency'].search([('name', '=', 'CLP')], limit=1)
		usd = self.env['res.currency'].search([('name', '=', 'USD')], limit=1)
		stage_invoiced = self.env['backlog.stages'].search([('stages_type', '=', 'invoiced')], limit=1)
		lines = self.env['sale.order.line'].search([('backlog_state_id', '!=', stage_invoiced.id)])
		today = fields.Date.context_today(self)
		for rec in lines:
			company = rec.order_id.company_id or self.env.company
			base = rec.price_total
			rec.clp_price_total = rec.currency_id._convert(base, clp, company, today)
			rec.amount_us = rec.currency_id._convert(base, usd, company, today)

	# Compute on Nivel Riesgo
	@api.depends('cpl_currency_id', 'price_total', 'currency_id', 'forecast_date', 'create_date', 'order_id.company_id')
	def compute_clp_currency(self):
		for rec in self:
			to_currency = rec.cpl_currency_id or self.env.ref('base.CLP', raise_if_not_found=False)
			if not to_currency:
				rec.clp_price_total = 0.0
				continue
			company = rec.order_id.company_id or self.env.company
			rate_date = rec.forecast_date or rec.create_date or fields.Date.context_today(self)
	...
	@api.depends('price_total', 'currency_id', 'forecast_date', 'create_date', 'order_id.company_id')
	def _compute_amount_us(self):
		usd = self.env['res.currency'].search([('name', '=', 'USD')], limit=1)
		for rec in self:
			rate_date = rec.forecast_date or rec.create_date or fields.Date.context_today(self)
			company = rec.order_id.company_id or self.env.company
			rec.amount_us = rec.currency_id._convert(rec.price_total, usd, company, rate_date)
	...
	def action_provision(self):
		stage_provisioned = self.env['backlog.stages'].search([('stages_type', '=', 'provisioned')], limit=1)
		for rec in self:
			rec.sudo().write({
				'income_recognition_date': fields.Date.context_today(self),
				'backlog_state_id': stage_provisioned.id if stage_provisioned else False,
				'bklg_state': 'provisioned',
			})

	def action_reverse_provision(self):
		stage_planning = self.env['backlog.stages'].search([('stages_type', '=', 'planning')], limit=1)
		for rec in self:
			rec.sudo().write({
				'income_recognition_date': fields.Date.context_today(self),
